...
describe('convertFragment()', () => {
  ...
  test('throws 404 for missing fragment', async () => {
    await expect(convertFragment('user@example.com', 'does-not-exist', 'html')).rejects.toMatchObject({
      status: 404,
    });
  });
  ...
  test('rejects non-markdown fragments with 415', async () => {
    ...
    await expect(convertFragment(plain.ownerId, plain.id, 'html')).rejects.toMatchObject({
      status: 415,
    });
  });
  ...
  test('rejects non-html target extensions with 415', async () => {
    ...
    await expect(convertFragment(md.ownerId, md.id, 'txt')).rejects.toMatchObject({
      status: 415,
    });
  });
});